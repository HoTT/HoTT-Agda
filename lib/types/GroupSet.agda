...
-- TODO Wait for changes in lib.types.Group.
module lib.types.GroupSet {i} {GEl : Type i} where

  -- The right group action with respect to the group [grp].
  record GsetStructure (grp : Group GEl) {j} (El : Type j)
    ...
    field
      act : El → GEl → El
      unit-r : ∀ x → act x G.ident == x
      assoc : ∀ x g₁ g₂ → act (act x g₁) g₂ == act x (G.comp g₁ g₂)

  -- The definition of a G-set.  A set [El] equipped with
  -- a right group action with respect to [grp].
  record Gset (grp : Group GEl) {j} : Type (lsucc (lmax i j)) where
    constructor gset
    field
      El : Type j
      El-level : is-set El
  ...
  module _ {grp : Group GEl} {j} {El : Type j} {El-level : is-set El} where
    private
      module G = Group grp
    open GsetStructure
    private
      gset-structure=' : ∀ {gss₁ gss₂ : GsetStructure grp El El-level}
        ...
        → unit-r gss₁ == unit-r gss₂
          [ (λ act → ∀ x → act x G.ident == x) ↓ act= ]
        → assoc gss₁ == assoc gss₂
          [ (λ act → ∀ x g₁ g₂ →
              act (act x g₁) g₂ == act x (G.comp g₁ g₂)) ↓ act= ]
        → gss₁ == gss₂
  ...
  module _ (grp : Group GEl) {j : ULevel} where
    private
      module G = Group grp
    open Gset {grp} {j}
    private
