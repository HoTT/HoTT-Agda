...
module Homotopy.VanKampen.SplitCode {i} (d : pushout-diag i)
  ...
  module _ where
    ...
    code-rec-nondep : ∀ {j}
      ...
      (h₁-a-rr : ∀ n {co} (pco : P-a (f $ loc n))
        → h₀-ba n {co a⟦ n ⟧b refl₀ _} (h₀-ab n {co} pco $ refl₀ _) (refl₀ _) ≡ pco)
      (h₁-b-rr : ∀ n {co} (pco : P-b (g $ loc n))
        → h₀-ab n {co b⟦ n ⟧a refl₀ _} (h₀-ba n {co} pco $ refl₀ _) (refl₀ _) ≡ pco)
      (h₁-ab-s : ∀ n₁ {co : code-a $ f $ loc n₁} (pco : P-a $ f $ loc n₁) n₂ (r : loc n₁ ≡₀ loc n₂) →
          h₀-ba n₂ {co a⟦ n₁ ⟧b ap₀ g r} (h₀-ab n₁ {co} pco $ ap₀ g r) (refl₀ _)
        ≡ h₀-ba n₁ {co a⟦ n₁ ⟧b refl₀ _} (h₀-ab n₁ {co} pco $ refl₀ _) (ap₀ f r))
      → (∀ {a₂} (co : code-a a₂) → P-a a₂)
      × (∀ {b₂} (co : code-b b₂) → P-b b₂)
  ...
  module _ where
    ...
    trans-ba (refl _) n co p = ap (ba n co) (! (refl₀-right-unit p))

    trans-ab : ∀ {y b} (q : y ≡ b) n co (p : g (loc n) ≡₀ y)
      → transport code-b q (co a⟦ n ⟧b p) ≡ co a⟦ n ⟧b p ∘₀ proj q
    trans-ab (refl _) n co p = ap (ab n co) (! (refl₀-right-unit p))
  ...
  module _ where
    ...
    code-a-rec {j} P-a ⦃ P-a-is-set ⦄ h₀-a h₀-ba h₁-a-rr = π₁ $ code-rec
      ...
      (λ n₁ {co} (pco : P-a co) n₂ r →
        transport (P-a {f $ loc n₂}) (code-ab-swap n₁ co n₂ r)
          (h₀-ba n₂ (co a⟦ n₁ ⟧b ap₀ g r) $ refl₀ _)
            ≡⟨ ! $ h₁-a-rr n₂ _ ⟩
        transport (P-a {f $ loc n₂}) (code-a-refl-refl n₂ _)
          (h₀-ba n₂ (_ a⟦ n₂ ⟧b refl₀ _) $ refl₀ _)
            ≡⟨ ap (λ x → transport (P-a {f $ loc n₂}) (code-a-refl-refl n₂ _)
                $ h₀-ba n₂ (_ a⟦ n₂ ⟧b refl₀ _) $ refl₀ _) $ code-ab-swap n₁ co n₂ r ⟩
        transport (P-a {f $ loc n₂}) (code-a-refl-refl n₂ _)
          (h₀-ba n₂ (_ a⟦ n₂ ⟧b refl₀ _) $ refl₀ _)
            ≡⟨ h₁-a-rr n₂ _ ⟩∎
        h₀-ba n₁ (co a⟦ n₁ ⟧b refl₀ _) (ap₀ f r)
            ∎)
  ...
  module _ where
    ...
    a⇒b n co = co a⟦ n ⟧b refl₀ _

    b⇒a : ∀ n → code-b (g $ loc n) → code-a (f $ loc n)
    b⇒a n co = co b⟦ n ⟧a refl₀ _
    ...
    abstract
      trans-code-a-r : ∀ n₁ n₂ (r : loc n₁ ≡ loc n₂) (co : code-a $ f $ loc n₁)
        → transport (code-a ◯ f) r co ≡ co a⟦ n₁ ⟧b refl₀ _ b⟦ n₁ ⟧a proj (ap f r)
      trans-code-a-r n₁ n₂ r co =
        ...
        transport code-a (ap f r) (co a⟦ n₁ ⟧b refl₀ _ b⟦ n₁ ⟧a refl₀ _)
          ≡⟨ trans-ba (ap f r) n₁ _ (refl₀ _) ⟩∎
    ...
    abstract
      ...
      trans-code-b-r n₁ n₂ r co =
        ...
        transport code-b (ap g r) (co b⟦ n₁ ⟧a refl₀ _ a⟦ n₁ ⟧b refl₀ _)
          ≡⟨ trans-ab (ap g r) n₁ _ (refl₀ _) ⟩∎
        co b⟦ n₁ ⟧a refl₀ _ a⟦ n₁ ⟧b proj (ap g r)
          ∎
    ...
    private
      abstract
        ...
        code-glue-eq-route n₁ n₂ r = equiv-eq $ funext λ co →
          ...
          transport (code-b ◯ g) r (co a⟦ n₂ ⟧b refl₀ _ b⟦ n₂ ⟧a proj (ap f $ ! r) a⟦ n₁ ⟧b refl₀ _)
              ≡⟨ ap (λ x → transport (code-b ◯ g) r $ x a⟦ n₁ ⟧b refl₀ _) $ ! $ code-ab-swap _ co _ (proj $ ! r) ⟩
          transport (code-b ◯ g) r (co a⟦ n₂ ⟧b proj (ap g $ ! r) b⟦ n₁ ⟧a refl₀ _ a⟦ n₁ ⟧b refl₀ _)
              ≡⟨ ap (transport (code-b ◯ g) r) $ code-b-refl-refl n₁ _ ⟩
          transport (code-b ◯ g) r (co a⟦ n₂ ⟧b proj (ap g $ ! r))
              ≡⟨ trans-code-b-r _ _ r _ ⟩
          co a⟦ n₂ ⟧b proj (ap g $ ! r) b⟦ n₁ ⟧a refl₀ _ a⟦ n₁ ⟧b proj (ap g r)
