...
module Homotopy.Extensions.ProductPushoutToProductToConnected where
  ...
  module _ {i j} {A B A′ B′ : Set i}
    ...
    connected-extend a′ b′ = π₁ (π₁ (π₁ extend-magic₂) a′) b′
 
    -- β rules
    abstract
      connected-extend-β-left : ∀ a′ b → connected-extend a′ (g b) ≡ left* a′ b
      connected-extend-β-left a′ b = ! $ π₂ (π₁ (π₁ extend-magic₂) a′) b

      connected-extend-β-right : ∀ a b′ → connected-extend (f a) b′ ≡ right* a b′
      connected-extend-β-right a b′ = ! $ happly (base-path (π₂ (π₁ extend-magic₂) a)) b′
