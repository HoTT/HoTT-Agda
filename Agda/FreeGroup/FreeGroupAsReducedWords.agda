...
is-reduced-is-prop (x ∷ (y ′∷ w)) = ×-hlevel 1 (pi-is-prop (λ _ → λ ())) (is-reduced-is-prop (y ′∷ w))
is-reduced-is-prop (x ′∷ ε) = unit-is-prop
is-reduced-is-prop (x ′∷ (y ∷ w)) = ×-hlevel 1 (pi-is-prop (λ _ → λ ())) (is-reduced-is-prop (y ∷ w))
is-reduced-is-prop (x ′∷ (y ′∷ w)) = is-reduced-is-prop (y ′∷ w)
...
word-total-path : {x y : A} (p : x ≡ y) {v w : word} (q : v ≡ w) → (x ∷ v ≡ y ∷ w)
...
word-dec-eq (x ∷ v) (y ∷ w) | inl x≡y | inr v≢w = inr (λ p → v≢w (word-fiber-path p))
...
reduced-to-freegroup ((x ∷ w) , r) = x · reduced-to-freegroup (w , tail-is-reduced x w r)
reduced-to-freegroup ((x ′∷ w) , r) = x ⁻¹· reduced-to-freegroup (w , tail'-is-reduced x w r)
...
mul-reduce x ((y ′∷ w) , red) | inr different = ((x ∷ (y ′∷ w)) , (different , red))
...
abstract
  mul-mul'-reduce : (x : A) (w : reduced-word) → mul-reduce x (mul'-reduce x w) ≡ w
  ...
  mul-mul'-reduce x ((y ∷ (z ′∷ w)) , red) | inl equal | inl absurd = abort-nondep (π₁ red (! equal ∘ absurd))
  mul-mul'-reduce x ((y ∷ (z ′∷ w)) , red) | inl equal | inr obvious = total-path (map _ equal) (π₁ (is-reduced-is-prop (y ∷ (z ′∷ w)) _ _))
  mul-mul'-reduce x ((y ∷ w) , red) | inr different with (eq x x)
  mul-mul'-reduce x ((y ∷ w) , red) | inr different | inl obvious = refl _
  mul-mul'-reduce x ((y ∷ w) , red) | inr different | inr absurd = abort-nondep (absurd (refl _))
  mul-mul'-reduce x ((y ′∷ w) , red) with (eq x x) 
  mul-mul'-reduce x ((y ′∷ w) , red) | inl obvious = refl _
  mul-mul'-reduce x ((y ′∷ w) , red) | inr absurd = abort-nondep (absurd (refl _))
...
abstract
  mul-reduce-reduced : (x : A) (w : word) (red : is-reduced (x ∷ w)) → mul-reduce x (w , tail-is-reduced x w red) ≡ ((x ∷ w) , red)
  ...
  mul-reduce-reduced x (y ′∷ w) red | inr obvious = total-path (refl _) (π₁ (is-reduced-is-prop (x ∷ (y ′∷ w)) _ _))
...
inv₁ (ε , red) = refl _
inv₁ ((x ∷ w) , red) = map (mul-reduce x) (inv₁ (w , tail-is-reduced x w red)) ∘ mul-reduce-reduced x w red
inv₁ ((x ′∷ w) , red) = map (mul'-reduce x) (inv₁ (w , tail'-is-reduced x w red)) ∘ mul'-reduce-reduced x w red

reduced-to-freegroup-mul-reduce : (x : A) (v : reduced-word) → reduced-to-freegroup (mul-reduce x v) ≡ x · (reduced-to-freegroup v)
...
inv₂ = freegroup-rec _
  (refl _)
  (λ x u p → reduced-to-freegroup-mul-reduce x (freegroup-to-reduced u) ∘ map (λ t → x · t) {y = u} p)
  (λ x u p → reduced-to-freegroup-mul'-reduce x (freegroup-to-reduced u) ∘ map (λ t → x ⁻¹· t) {y = u} p)
...
freegroup-equiv-reduced = (freegroup-to-reduced , iso-is-eq _ reduced-to-freegroup inv₁ inv₂)
