...
-- We only consider the universal property internally to a fixed universe [Set i]. If we don’t, we
-- would have to consider an universe polymorphic [P] and I don’t want to quantify over universe polymorphic things
...
pullback-to-cocone : (top : Set i) → (pullback (top → A) (top → B) (top → C) (λ h → f ◯ h) (λ h → g ◯ h) → cocone top)
pullback-to-cocone top (top→A , top→B , h) = (top→A , top→B , happly h)

cocone-to-pullback : (top : Set i) → (cocone top → pullback (top → A) (top → B) (top → C) (λ h → f ◯ h) (λ h → g ◯ h))
...
compose-cocone-map : (D E : Set i) (Dcocone : cocone D) → ((f : E → D) → cocone E)
compose-cocone-map D E (top→A , top→B , h) f = ((top→A ◯ f) , (top→B ◯ f) , λ x → h (f x))

is-pullback : (D : Set i) ⦃ PD : P D ⦄ (Dcocone : cocone D) → Set _
is-pullback D Dcocone = (E : Set i) ⦃ PE : P E ⦄ → is-equiv (compose-cocone-map D E Dcocone)
