...
contr-contr-equiv : {i j : Level} {A : Set i} {B : Set j} (f : A → B) (cA : is-contr A) (cB : is-contr B) → is-equiv f
...
is-hlevel : (n : ℕ) {i : Level} (A : Set i) → Set i
...
-- hlevel-equiv : (n : ℕ) {i : Level} (A : Set i) → (is-hlevel' n A → is-hlevel n A)
-- hlevel-equiv O A p = p
-- hlevel-equiv (S O) A p = λ x y → π₁ (p x y)
-- hlevel-equiv (S (S n)) A p = λ x y → hlevel-equiv (S n) (x ≡ y) (p x y)

-- hlevel-equiv2 : (n : ℕ) {i : Level} (A : Set i) → (is-hlevel n A → is-hlevel' n A)
...
is-prop : {i : Level} (A : Set i) → Set _
is-prop = is-hlevel 1

is-set : {i : Level} (A : Set i) → Set _
is-set = is-hlevel 2

is-gpd : {i : Level} (A : Set i) → Set _
is-gpd = is-hlevel 3

is-prop-is-contr : {i : Level} (A : Set i) → is-prop (is-contr A)
is-prop-is-contr A x y = total-path (π₂ y (π₁ x)) (funext-dep _ _
  ...
  lemma-is-prop-is-contr : {i : Level} (A : Set i) (c : is-contr A) {x y : A} (p : x ≡ y) → p ∘ π₂ c y ≡ π₂ c x
  lemma-is-prop-is-contr A c (refl _) = refl _

is-prop-pi : {i j : Level} (A : Set i) (P : A → Set j) (p : (x : A) → is-prop (P x)) → is-prop ((x : A) → P x)
is-prop-pi A P p f g = funext-dep _ _ (λ x → p x (f x) (g x))

lemma1-is-prop-is-prop : {i : Level} (A : Set i) (c : is-prop A) {x y : A} (p : x ≡ y) → p ∘ c y y ≡ c x y
lemma1-is-prop-is-prop A c (refl _) = refl _

-- We apply the previous lemma with [p = c y y]
lemma2-is-prop-is-prop : {i : Level} (A : Set i) (c : is-prop A) (y : A) → c y y ≡ refl y
...
lemma3-is-prop-is-prop : {i : Level} (A : Set i) (c : is-prop A) {x y : A} (p : x ≡ y) → p ≡ c x y
lemma3-is-prop-is-prop A c (refl _) = ! (lemma2-is-prop-is-prop A c _)

is-prop-is-prop : {i : Level} (A : Set i) → is-prop (is-prop A)
is-prop-is-prop A c c' = funext-dep _ _ (λ x → funext-dep _ _ (λ x' → lemma3-is-prop-is-prop _ c' (c x x')))

is-prop-hlevel : (n : ℕ) {i : Level} (A : Set i) → is-prop (is-hlevel n A)
...
is-increasing-hlevel : (n : ℕ) {i : Level} (A : Set i) → is-hlevel n A → is-hlevel (S n) A
