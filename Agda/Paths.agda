...
map : ∀ {i j} {A : Set i} {B : Set j} (f : A → B) {x y : A} (p : x ≡ y) → f x ≡ f y
map f (refl _) = refl _

map-dep : ∀ {i j} {A : Set i} {P : A → Set j} (f : (a : A) → P a) {x y : A} (p : x ≡ y) → transport P p (f x) ≡ f y
map-dep f (refl _) = refl _

map-dep! : ∀ {i j} {A : Set i} {P : A → Set j} (f : (a : A) → P a) {x y : A} (p : x ≡ y) → f x ≡ transport P (! p) (f y)
map-dep! f (refl _) = refl _
...
module GpdStruct {i} {A : Set i} where
  ...
  -- To remember the order, [opposite-concat …] gives a result of the form [opposite (concat …) ≡ …]
...
module TransportReductionRules {i} {A : Set i} where
  -- This first part is about transporting something in a known fibration. In the names,
  -- [x] represents the variable of the fibration, [a] is a constant term, [A] is a
  -- constant type, and [f] and [g] are constant functions.
  ...
  trans-A→Pxy : ∀ {j k} (B : Set j) (P : (x : A) (y : B) → Set k) {b c : A} (p : b ≡ c) (q : (y : B) → P b y) (a : B)
  ...
  trans-totalpath {j} {k} {P} {Q} {(x₁ , x₂)} {(y₁ , y₂)} p q f = trans-totalpath' {j} {k} {P} {Q} {x₁} {y₁} {x₂} {y₂} p q f where
    ...
    trans-totalpath' : ∀ {j k} {P : A → Set j} {Q : Σ A P → Set k} {x₁ y₁ : A} {x₂ : P x₁} {y₂ : P y₁}
  ...
  homotopy-naturality : ∀ {j} {B : Set j} (f g : A → B) (p : (x : A) → f x ≡ g x) {x y : A} (q : x ≡ y) → map f q ∘ p y ≡ p x ∘ map g q
  ...
  opposite-map : ∀ {j} {B : Set j} (f : A → B) {x y : A} (p : x ≡ y) → ! (map f p) ≡ map f (! p)
  opposite-map f (refl _) = refl _

  map-opposite : ∀ {j} {B : Set j} (f : A → B) {x y : A} (p : x ≡ y) → map f (! p) ≡ ! (map f p)
  map-opposite f (refl _) = refl _

  concat-map : ∀ {j} {B : Set j} (f : A → B) {x y z : A} (p : x ≡ y) (q : y ≡ z) → map f p ∘ map f q ≡ map f (p ∘ q)
  concat-map f (refl _) _ = refl _

  map-concat : ∀ {j} {B : Set j} (f : A → B) {x y z : A} (p : x ≡ y) (q : y ≡ z) → map f (p ∘ q) ≡ map f p ∘ map f q
  map-concat f (refl _) _ = refl _

  compose-map : ∀ {j k} {B : Set j} {C : Set k} (g : B → C) (f : A → B) {x y : A} (p : x ≡ y) → map g (map f p) ≡ map (g ◯ f) p
  compose-map f g (refl _) = refl _

  map-compose : ∀ {j k} {B : Set j} {C : Set k} (g : B → C) (f : A → B) {x y : A} (p : x ≡ y) → map (g ◯ f) p ≡ map g (map f p)
  ...
  -- Add a [0] after [move] if the big path is constant, a [1] if the other small path is constant and then a [!] if the path you will move is an opposite
  --
  -- I’m not sure all of these functions are useful, but it can’t hurt to have them
  ...
  move-transp-left : ∀ {j} (P : A → Set j) {x y : A} (u : P y) (p : x ≡ y) (v : P x)
    → transport P (! p) u ≡ v → u ≡ transport P p v
  move-transp-left P _ (refl _) _ p = p
  
  move-transp-right : ∀ {j} (P : A → Set j) {x y : A} (p : y ≡ x) (u : P y) (v : P x)
    → u ≡ transport P (! p) v → transport P p u ≡ v
  move-transp-right P (refl _) _ _ p = p
  
  move!-transp-left : ∀ {j} (P : A → Set j) {x y : A} (u : P y) (p : y ≡ x) (v : P x)
    → transport P p u ≡ v → u ≡ transport P (! p) v
  move!-transp-left P _ (refl _) _ p = p
  
  move!-transp-right : ∀ {j} (P : A → Set j) {x y : A} (p : x ≡ y) (u : P y) (v : P x)
    → u ≡ transport P p v → transport P (! p) u ≡ v
  move!-transp-right P (refl _) _ _ p = p
