...
module WedgeCircles.LoopSpaceWedgeOfCircles2 {i} (A : Set i) (eq : dec-eq A) where
...
import WedgeCircles.LoopSpaceWedgeOfCircles

open WedgeCircles.LoopSpaceWedgeOfCircles A eq
...
-- We can now prove that [tot-type] is contractible using [CA-rec] (because [CA-rec]
-- means that [tot-type] is the Cayley graph of FA)
P-CA-contr : (x : tot-type) → Set _
P-CA-contr x = x ≡ CA-z e

-- We first prove it on points coming from reduced words
CA-contr-base-reduced-word : (w : reduced-word) → P-CA-contr (CA-z (reduced-to-freegroup w))
...
CA-contr-base u = map CA-z (! (inv₂ u)) ∘ (CA-contr-base-reduced-word (freegroup-to-reduced u))

-- Now we prove that it’s true on paths, coming from reduced words
abstract
  CA-contr-loop-reduced-word : (t : A) (w : reduced-word) →
    (map CA-z (reduced-to-freegroup-mul-reduce t w) ∘ ! (CA-e t (reduced-to-freegroup w))) ∘ (CA-contr-base-reduced-word w)
  ...
  CA-contr-loop-reduced-word x ((.x ′∷ w) , red) | inl (refl .x) =
    --  With the notations below, what we have to prove is [ (map CA-z (! p) ∘ ! q) ∘ ((q ∘ map CA-z p) ∘ r) ≡ r ]
...
CA-contr-loop : (t : A) (u : FA) → transport P-CA-contr (CA-e t u) (CA-contr-base u) ≡ (CA-contr-base (t · u))
CA-contr-loop t u =
  -- Idea:
  --
  -- We need to prove [p u ∘ (map CA-z (q u) ∘ for-red u) ≡ map CA-z (q (t · u)) ∘ for-red (t · u)]
  -- [CA-contr-loop-reduced-word] gives that [ (map CA-z comp ∘ p (k u)) ∘ for-red u ≡ for-red (t · u)]
  ...
  ∘ (whisker-right (for-red u) {q = p u ∘ map CA-z (q u)}
    ...
    ∘ (whisker-right (p (k u)) {q = map f (q u)}
      ...
      (map-compose CA-z (λ u₁ → t · u₁) (q u) ∘ (map (map CA-z) (π₁ (freegroup-is-set _ _ _ _)) ∘ map-concat CA-z (q (t · u)) comp))
...
total-equiv = contr-equiv-contr (total-map fiberwise-map) tot-path-is-contr tot-type-is-contr
