...
module TotalMapEquiv (e : is-equiv total-map) where
  ...
  -- Here is the fiberwise inverse, we use the inverse of the total map and transform it into a fiberwise map using [base-path-inverse]
  inv : ((x : A) → (Q x → P x))
  ...
  inv-left-inverse x y = map (λ u → transport P (base-path (inverse-right-inverse total-equiv (x , f x y))) u) (lemma2 x y)
    ...
    lemma2 x y = ! (fiber-path (! (inverse-left-inverse total-equiv (x , y))))
      ∘ map (λ p → transport P p y) (map-opposite π₁ (inverse-left-inverse total-equiv (x , y)) ∘ ! (map ! (lemma1 x y)))
