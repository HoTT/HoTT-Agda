...
eq-to-path-right-inverse : ∀ {i} {A B : Set i} (f : A ≃ B) → path-to-eq (eq-to-path f) ≡ f
...
trans-X : ∀ {i} {A B : Set i} (f : A ≡ B) (u : A) → transport (λ X → X) f u ≡ ((path-to-eq f) $ u)
trans-X (refl _) u = refl u

trans-eq-to-path : ∀ {i} {A B : Set i} (f : A ≃ B) (u : A) → transport (λ X → X) (eq-to-path f) u ≡ (f $ u)
trans-eq-to-path {i} {A} {B} f u = trans-X (eq-to-path f) u ∘ map (λ (t : A ≃ B) → t $ u) (eq-to-path-right-inverse f)

-- Induction along equivalences

equiv-induction : ∀ {i j} (P : (A : Set i) (B : Set i) (f : A ≃ B) → Set j) (d : (A : Set i) → P A A (id-eq A)) →
...
equiv-induction P d A B f = transport (P A B) (eq-to-path-right-inverse f) (equiv-induction-int P d A B (eq-to-path f)) where
  ...
  equiv-induction-int : ∀ {i j} (P : (A : Set i) (B : Set i) (f : A ≃ B) → Set j) (d : (A : Set i) → P A A (id-eq A)) →
