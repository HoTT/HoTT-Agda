...
module stash.modalities.JoinAdj where
  ...
  module _ {i j k} {A : Type i} {B : Type j} {C : Type k} where
    ...
    to-from (f , γ) = pair= idp (λ= eq)

      where eq : (a : A) → from (f , γ) (left a) , λ= (λ b → ap (from (f , γ)) (glue (a , b))) == γ a
            eq a = pair= idp (ap λ= (λ= (λ b → from-β f γ a b)) ∙ ! (λ=-η (snd (γ a))))
            
    from-to : (φ : A * B → C) → from (to φ) == φ
    from-to φ = λ= lem

      where lem : (x : A * B) → from (to φ) x == φ x
            lem = Join-elim (λ a → idp) (λ b → idp) (λ ab → ↓-==-in (coh ab))
