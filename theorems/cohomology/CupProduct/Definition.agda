...
smin-map-⊙∧-swap : ∀ {j k} (Y : Ptd j) (Z : Ptd k)
  (f : X ⊙→ Y)
  (g : X ⊙→ Z)
  → ⊙∧-swap Z Y ⊙∘ smin-map g f == smin-map f g
smin-map-⊙∧-swap Y Z f g =
  ⊙λ=' (λ x → idp) $
  ap (∧-swap Z Y) (ap2 smin (snd g) (snd f)) ∙ idp
    =⟨ ∙-unit-r (ap (∧-swap Z Y) (ap2 smin (snd g) (snd f))) ⟩
  ap (∧-swap Z Y) (ap2 smin (snd g) (snd f))
    =⟨ ap-ap2 (∧-swap Z Y) smin (snd g) (snd f) ⟩
  ap2 (λ x y → smin y x) (snd g) (snd f)
    =⟨ =ₛ-out $ ap2-out (λ x y → smin y x) (snd g) (snd f) ⟩
  ap (smin (fst f (pt X))) (snd g) ∙ ap (λ y → smin y (pt Z)) (snd f)
    =⟨ ! $ =ₛ-out $ ap2-out' smin (snd f) (snd g) ⟩
  ...
  abstract
    ⊙Ω∧-cp-comm : ∀ (m n : ℕ)
      → ⊙transport (λ k → ⊙Ω (⊙EM H⊗G.abgroup (S k))) (+-comm m n) ◃⊙∘
        ...
        ⊙Ω∧-cp H G n m ◃⊙∘
        ⊙∧-swap (⊙Ω (⊙EM G (S m))) (⊙Ω (⊙EM H (S n))) ◃⊙idf
    ⊙Ω∧-cp-comm m n =
      ⊙transport (λ k → ⊙Ω (⊙EM H⊗G.abgroup (S k))) (+-comm m n) ◃⊙∘
      ⊙Ω-fmap (⊙EM-fmap G⊗H.abgroup H⊗G.abgroup G⊗H.swap (S (m + n))) ◃⊙∘
      ⊙Ω∧-cp G H m n ◃⊙idf
      ...
      ⊙∧-fmap (⊙–> (spectrum G m)) (⊙–> (spectrum H n)) ◃⊙idf
        =⊙∘⟨ 3 & 2 & =⊙∘-in {gs = ⊙∧-fmap (⊙–> (spectrum H n)) (⊙–> (spectrum G m)) ◃⊙∘
                                  ⊙∧-swap (⊙Ω (⊙EM G (S m))) (⊙Ω (⊙EM H (S n))) ◃⊙idf} $
                     ! $ ⊙∧-swap-naturality (⊙–> (spectrum G m)) (⊙–> (spectrum H n)) ⟩
      ⊙Ω-fmap (⊙cond-neg H⊗G.abgroup (S (n + m)) (and (odd m) (odd n))) ◃⊙∘
      ⊙<– (spectrum H⊗G.abgroup (n + m)) ◃⊙∘
      ⊙∧-cp H G n m ◃⊙∘
