...
module cw.cohomology.ChainComplex2 {i : ULevel} where
  ...
  abstract
    test₀ : ∀ {n} (skel : Skeleton {i} (S n)) dec fin-sup {m} m<n Sm<n
    ...
    test₀ skel dec fin-sup m<n Sm<n = group-hom= $
  ...
  abstract
    test₁ : ∀ {n} (skel : Skeleton {i} (S (S n))) dec fin-sup
      → boundary-hom-template {n = S (S n)} skel dec fin-sup (inl (lteSR lteS)) (inl lteS)
  ...
  abstract
    test₂ : ∀ {n} (skel : Skeleton {i} (S n)) dec fin-sup
      → boundary-hom-template {n = S n} skel dec fin-sup (inl lteS) (inl lteE)
        == FreeAbGroup-extend (FreeAbGroup (cells-last (cw-init skel)))
          (boundary'-last skel dec fin-sup)
    test₂ skel dec fin-sup = group-hom= $
      ap (boundary-nth-template skel dec fin-sup lteS lteE idp) (path-lemma₂ skel)
