...
module cw.cohomology.ChainComplex {i : ULevel} where
  ...
  abstract
    ...
    boundary-pres-comp-nth-template skel dec fin-sup m≤n Sm≤n path₀ path₁ =
      ∘-pres-comp
  ...
  boundary-hom-nth-template skel dec fin-sup m≤n Sm≤n path₀ path₁ = record {
    f = boundary-nth-template skel dec fin-sup m≤n Sm≤n path₀ path₁;
    pres-comp = boundary-pres-comp-nth-template skel dec fin-sup m≤n Sm≤n path₀ path₁}

  boundary-hom-template : ∀ {n} (skel : Skeleton {i} n) dec
    → has-degrees-with-finite-supports skel dec
    → {m : ℕ} (m≤n? : Dec (m ≤ n)) (Sm≤n? : Dec (S m ≤ n))
    → (AbGroup.grp (chain-template skel Sm≤n?) →ᴳ AbGroup.grp (chain-template skel m≤n?))
  boundary-hom-template skel dec fin-sup _ (inr _) = cst-hom
  boundary-hom-template skel dec fin-sup (inr m≰n) (inl Sm≤n) = ⊥-rec $ m≰n (≤-trans lteS Sm≤n)
  boundary-hom-template skel dec fin-sup (inl m≤n) (inl Sm≤n) =
    boundary-hom-nth-template skel dec fin-sup m≤n Sm≤n (cw-init-take Sm≤n skel)
  ...
  chain-complex {n} skel dec fin-sup = record {M} where
    module M where
      ...
      boundary m = boundary-hom-template skel dec fin-sup (≤-dec m n) (≤-dec (S m) n)
