...
open import HoTT
import homotopy.ConstantToSetExtendsToProp
...
module Exact {i j k} {G : Group i} {H : Group j} {K : Group k}
  {φ : G →ᴳ H} {ψ : H →ᴳ K} (exact : is-exact φ ψ) where
    ...
    G-trivial-implies-ψ-is-inj G-is-triv =
      has-trivial-ker-is-injᴳ ψ λ h ψh=0 →
        Trunc-rec (H.El-is-set _ _)
          ...
          (ker-sub-im exact h ψh=0)

  G-to-ker : G →ᴳ Ker.grp ψ
  G-to-ker = Ker.inject-lift ψ φ (λ g → im-sub-ker exact (φ.f g) [ g , idp ])
    ...
    φ-inj-implies-G-to-ker-is-equiv φ-is-inj = is-eq _ from to-from from-to
      where
        ...
        module From (k : Ker.El ψ)
          = homotopy.ConstantToSetExtendsToProp
              {A = hfiber φ.f (fst k)} {B = G.El}
              G.El-is-set (λ hf → fst hf)
              (λ hf₁ hf₂ → φ-is-inj _ _ (snd hf₁ ∙ ! (snd hf₂)))

        from : Ker.El ψ → G.El
        from = λ k → From.cst-extend k (uncurry (ker-sub-im exact) k)
        ...
        to-from k = Subtype=-out (Ker.P.subEl-prop ψ) $
          Trunc-elim
            ...
            (λ _ → H.El-is-set _ _)
            (λ{(g , p) → p})
            (uncurry (ker-sub-im exact) k)

        from-to : ∀ g → from (to g) == g
        from-to g = From.cst-extend-is-const (to g) (uncurry (ker-sub-im exact) (to g)) [ g , idp ]

    K-trivial-implies-φ-surj : is-trivialᴳ K → is-surjᴳ φ
    K-trivial-implies-φ-surj K-is-triv h = ker-sub-im exact h (K-is-triv (ψ.f h))
  ...
  coker-to-K H-is-abelian = record {
    f = SetQuot-rec K.El-is-set
      (λ h → ψ.f h)
      (λ {h₁} {h₂} h₁h₂⁻¹-in-im →
        K.zero-diff-same _ _ $ ! (ψ.pres-diff h₁ h₂) ∙ im-sub-ker exact (H.diff h₁ h₂) h₁h₂⁻¹-in-im);
