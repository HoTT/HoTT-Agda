...
CF-ident : (n : ℤ) {X : Ptd i} → CF-hom n (⊙idf X) == idhom (C n X)
...
C-exact : (n : ℤ) {X Y : Ptd i} (f : fst (X ⊙→ Y))
  → is-exact (GroupHom.⊙f (CF-hom n (⊙cfcod f)))
             (GroupHom.⊙f (CF-hom n f))
...
C-dimension : (n : ℤ) → n ≠ O → C n (⊙Sphere 0) == 0G
